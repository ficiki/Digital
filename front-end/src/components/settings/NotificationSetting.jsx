import { Bell, WifiOff, Save, X, CheckCircle, AlertCircle, BellRing } from 'lucide-react';
import { subscribeUserToPush } from '../../utils/pushNotifications';

const NotificationSetting = () => {
  const { preferences, loading, error, updatePreferences, fetchPreferences } = useNotification();
  ...
  if (error && !localPreferences) { // Only show error if preferences couldn't be loaded at all
    return (
      <div className="flex items-center justify-center py-8 text-red-500">
        <WifiOff className="h-6 w-6 mr-2" />
        <p>Gagal memuat preferensi notifikasi. {error.message || 'Coba lagi nanti.'}</p>
      </div>
    );
  }
  
  // Render nothing or a placeholder if preferences are not loaded yet and not in an error state.
  if (!localPreferences) {
      return null;
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-medium text-gray-700 dark:text-gray-300">Notifikasi Perangkat (Push)</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Aktifkan untuk menerima notifikasi langsung di perangkat Anda, bahkan saat browser ditutup.
            </p>
          </div>
          <button
      ...
      <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <button
          onClick={handleSave}
          disabled={loading || !localPreferences}
