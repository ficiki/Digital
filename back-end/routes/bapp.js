...
router.get('/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const document = {
      ...
      judul: baseDocument.nomor_bapp, // Use nomor_bapp as judul for now
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO document_history (
        jenis_dokumen, id_dokumen, actor_role, actor_id, actor_name,
        aktivitas, keterangan, status_sebelum, status_sesudah
      ) VALUES ('bapp', ?, 'direksi', ?, ?, 'approved', ?, 'submitted', 'approved_direksi')`,
      [id, req.user.id, req.user.nama_lengkap, catatan || 'Disetujui oleh direksi']
    );
    await pool.query('COMMIT');
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO document_history (
        jenis_dokumen, id_dokumen, actor_role, actor_id, actor_name,
        aktivitas, keterangan, status_sebelum, status_sesudah
      ) VALUES ('bapp', ?, 'direksi', ?, ?, 'rejected', ?, 'submitted', 'rejected')`,
      [id, req.user.id, req.user.nama_lengkap, catatan]
    );
    await pool.query('COMMIT');
